...
const RailNavLink = ({ link, pathname, onShowTip, onHideTip, isNight }) => {
  ...
  return (
    <Link
      ...
      className={`group relative flex w-full flex-col items-center gap-1 rounded-xl px-1 py-2.5 transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#7440E9] focus-visible:ring-offset-2 ${
        isNight
  ...
  return (
    <>
      ...
      <motion.nav
        ...
        className={`font-cairo fixed inset-y-0 right-0 z-[1000] hidden w-24 flex-col backdrop-blur-xl lg:flex ${isNight ? "nav-rail-night border-l" : "border-l border-purple-200/50 bg-white/95 shadow-[0_0_24px_rgba(116,64,233,0.08)]"}`}
        dir="rtl"
        aria-label="التنقل الرئيسي"
      >
        <motion.div
          className={`flex shrink-0 flex-col items-center gap-1 border-b px-1 py-1 ${isNight ? "border-white/10" : "border-purple-100/80"}`}
        >
          <Link
            to="/"
            onMouseEnter={(e) => showRailTip("مشكاة — الرئيسية", e)}
            onMouseLeave={scheduleHideRailTip}
            className={`group flex w-full flex-col items-center gap-1 rounded-xl transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#7440E9] ${isNight ? "hover:bg-[#2c2c31]" : "hover:bg-purple-50"}`}
            aria-label="الرئيسية"
          >
            <img
              src="/logo.svg"
            ...
            />
            <span
              className={`text-[10px] font-bold ${isNight ? "text-zinc-300" : "text-[#7440E9]"}`}
            >
              مشكاة
            </span>
          </Link>
        ...
        >
          <ThemeToggleBtn
            className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all active:scale-95 ${isNight ? "border border-white/10 bg-[#2c2c31] hover:bg-[#34343a]" : "border border-purple-100 bg-purple-50 hover:bg-purple-100"}`}
          />
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div
                    ...
                    className="absolute bottom-0 right-full z-[1200] mr-2 w-48 overflow-hidden rounded-2xl border border-purple-200/50 bg-white/98 py-2 text-right shadow-2xl backdrop-blur-xl"
                  >
                    <Link
                      to="/profile"
                      className="block px-4 py-3 text-sm text-gray-700 transition hover:bg-gradient-to-l hover:from-purple-50 hover:to-indigo-50 hover:text-[#7440E9]"
                    ...
                    <button
                      ...
                      className="w-full px-4 py-3 text-right text-sm text-gray-700 transition hover:bg-red-50 hover:text-red-600"
                    >
                      تسجيل الخروج
                    </button>
                  </motion.div>
      ...
      {/* ——— الجوال: يسار صورة المستخدم | وسط اللوجو | يمين القائمة ——— */}
      <nav
        className={`font-cairo fixed top-0 right-0 left-0 z-[1000] grid h-[3.75rem] grid-cols-[3.25rem_1fr_2.75rem_3.25rem] items-center border-b px-2 lg:hidden ${isNight ? "nav-mobile-night" : "border-purple-200 bg-white shadow-[0_2px_12px_rgba(116,64,233,0.08)]"}`}
        dir="ltr"
      >
        {/* يسار الشاشة — صورة المستخدم */}
          ...
          {isAuthenticated && user ? (
            ...
            >
              <img
                src={avatarUrl}
                alt=""
                className="h-10 w-10 rounded-full border-2 border-[#7440E9] object-cover"
              />
            </Link>
          ) : (
            <Link
              to="/login"
              className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-dashed border-purple-200 bg-purple-50 text-[#7440E9] transition-colors hover:bg-purple-100"
              aria-label="تسجيل الدخول"
            >
              <img
        ...
        <div className="flex justify-center">
          <ThemeToggleBtn
            className={`flex h-9 w-9 items-center justify-center rounded-lg transition-colors ${isNight ? "bg-[#2c2c31]" : "bg-purple-50"}`}
          />
        </div>

        {/* يمين الشاشة — قائمة الروابط */}
        <div className="flex justify-end">
          <button
            type="button"
            className={`flex h-10 w-10 items-center justify-center rounded-xl border transition-colors active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-500 ${isNight ? "border-white/10 bg-[#2c2c31] text-zinc-300 hover:bg-[#34343a]" : "border-purple-100 bg-purple-50 text-[#7440E9] hover:bg-purple-100"}`}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[1001] bg-black/50 lg:hidden"
            onClick={closeMobile}
          >
            <motion.div
              initial={{ x: "100%" }}
            ...
            >
              <div className="flex items-center justify-between border-b border-purple-200/50 p-4">
                <span className="font-['MeshkahFont'] text-lg font-bold text-[#7440E9]">
                  القائمة
                </span>
                <button
                  type="button"
                  className="rounded-xl p-2 text-[#7440E9] hover:bg-purple-50"
                  onClick={closeMobile}
                  aria-label="إغلاق"
                >
                  <X className="h-6 w-6" />
              ...
              {isAuthenticated && user && (
                <div className="border-b border-purple-200/50 p-4">
                  <Link
                    to="/profile"
                    onClick={closeMobile}
                    className="flex items-center gap-3 rounded-2xl bg-gradient-to-l from-purple-50 to-indigo-50 p-3"
                  >
                    <img
                      src={avatarUrl}
                      alt=""
              ...
              <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
                {NAV_SECTIONS.map((section) => (
                  <div key={section.title} className="space-y-2">
                    <h3 className="px-1 text-xs font-bold uppercase tracking-wide text-[#7440E9]">
                      {section.title}
                    </h3>
              ...
              <div className="space-y-3 border-t border-purple-200/50 p-4">
                <Link
                  to="/privacy-policy"
                  onClick={closeMobile}
                  className="block text-center text-sm text-gray-500 hover:text-[#7440E9]"
                >
                  سياسة الخصوصية
                </Link>
                {isAuthenticated ? (
