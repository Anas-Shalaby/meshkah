...
const TestResultsView = ({ results, test }) => {
  if (!results) return null;

  // Handle different data structures
  const attempt = results.attempt || results;
  const questions = results.questions || [];
  const testInfo = test || results.test || { title: "اختبار اليوم" };

  // Check if attempt exists and has percentage
  if (
    !attempt ||
    (attempt.percentage === undefined && attempt.score === undefined)
  ) {
    return (
