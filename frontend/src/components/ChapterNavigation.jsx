...
const ChapterNavigation = ({ bookSlug, chapterId, language, onChapterChange }) => {
  ...
  const fetchNavigation = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/islamic-library/local-books/${bookSlug}/chapters/${chapterId}/navigation`
      );
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => window.location.href = `/islamic-library/local-books/${bookSlug}`}
