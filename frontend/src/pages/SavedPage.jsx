...
const SavedPage = () => {
  ...
  const seoData = {
    ...
    description:
      "استكشف أحاديثك المحفوظة والمفضلة في مشكاة. تصفح، ابحث، وشارك أحاديثك المفضلة مع إمكانية التنظيم في مجموعات.",
    keywords:
      "أحاديث محفوظة, أحاديث مفضلة, مشكاة, حديث نبوي, حفظ أحاديث, مجموعات أحاديث, مشاركة أحاديث",
  ...
  const uniqueCollections = [
    ...
    ...Array.from(
      new Set(
        bookmarkedHadiths.map((h) => h.bookmarkCollection || "الافتراضي"),
      ),
    ),
  ];
      ...
      <div className={`relative min-h-screen overflow-hidden ${t.page}`}>
        ...
        <div className="relative z-10">
          ...
          <div className="max-w-6xl mx-auto px-4 py-6">
            ...
            >
              <div
                className={`inline-block rounded-2xl px-6 py-3 shadow-lg ${isNight ? "border border-white/10 bg-[#242428]/95" : "border border-purple-200/50 bg-white/80 backdrop-blur-xl"}`}
              >
                <p className={`text-lg font-medium sm:text-xl ${t.textAccent}`}>
                  {quote}
                </p>
              </div>
            </motion.div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
              ...
              >
                <motion.div
                  className={`flex h-12 w-12 items-center justify-center rounded-2xl shadow-lg ${t.iconBox}`}
                >
                  <Bookmark className="w-6 h-6 text-white" />
                </motion.div>
                <div>
                  <h1
                    className={`text-2xl font-bold sm:text-3xl ${t.textHeading}`}
                  >
                    الأحاديث المحفوظة
                  </h1>
                  <p className={`text-sm ${t.textBody}`}>
                    استكشف أحاديثك المفضلة
                  </p>
                </div>
              </motion.div>
              ...
              >
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 shadow-lg transition-all duration-300 ${t.ghostBtn}`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-2 rounded-xl px-4 py-2 shadow-lg transition-all duration-300 ${
                    readingMode
                      ? isNight
                        ? "bg-[#5b4d6f] text-white"
            ...
            >
              <div className="relative max-w-2xl mx-auto">
                ...
                <input
                  ...
                  className={`w-full py-3 pr-10 pl-4 ${t.input}`}
                />
                {searchTerm && (
                  <button
              ...
              <p className={`mt-2 text-center text-xs ${t.textMuted}`}>
                يمكنك البحث باسم الراوي أو نص الحديث أو التصنيف أو الدرجة، بدون
                الحاجة للتشكيل
              </p>
            </motion.div>
            ...
            <AnimatePresence>
              {showFilters && (
                <motion.div
                  ...
                  className={`mb-6 rounded-2xl p-4 shadow-lg ${isNight ? "border border-white/10 bg-[#242428]/95" : "border border-purple-200/50 bg-white/80 backdrop-blur-xl"}`}
                >
                  <div className="flex flex-col sm:flex-row gap-4">
                    {/* Collection Filter */}
                    <div className="flex-1">
                      <label
                        className={`mb-2 block text-sm font-semibold ${t.textHeading}`}
                      >
                        المجموعات
                      </label>
                        ...
                        {uniqueCollections.map((col) => (
                          <button
                            ...
                            className={`px-3 py-1.5 transition-all duration-300 ${
                              selectedCollection === col ? t.chipActive : t.chip
                            }`}
                          >
                    ...
                    <div className="flex-1">
                      ...
                      <select
                        ...
                        className={`w-full rounded-lg px-3 py-2 focus:outline-none focus:ring-2 ${isNight ? "border border-white/10 bg-[#2c2c31] text-zinc-200 focus:ring-zinc-500" : "border border-purple-200 bg-white/80 text-gray-700 focus:ring-purple-500"}`}
                      >
